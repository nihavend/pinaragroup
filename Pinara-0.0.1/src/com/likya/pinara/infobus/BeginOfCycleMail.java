...
public class BeginOfCycleMail extends MultipartMail {
	
	public BeginOfCycleMail(String virtualId, HashMap<String, JobImpl> jobQueue) {
		try {
			// Instance name yanlış, artık tek bir senaryo yok !
			setMailSubject(Pinara.getMessage("BeginOfCycleMail.0") + virtualId /*Pinara.getInstance().getConfigurationManager().getPinaraConfig().getInstanceName()*/ + Pinara.getMessage("BeginOfCycleMail.1"));
			setMultipart(prepareBreginOfCycleMail(jobQueue));
	...
	private Multipart prepareBreginOfCycleMail(HashMap<String, JobImpl> jobQueue) throws MessagingException, URISyntaxException {
		...
		String mailHtml = MailContentHelper.getHTMLMailForStartup(MailSubjectType.BEGINOFCYCLE, jobQueue);
