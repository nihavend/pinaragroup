...
public class ValidPlatforms {
	...
	/**
	 * Aslında aşağıdaki gibi bir liste olmamalı. Sadece deploy edilecek sisteme ait
	 * lisans olmalı
	 */
	public static String[] getCommand(String jobCommand, boolean isUnixShScript) {

		// if(TlosServer.getTlosParameters().getJobCommand() != null &&
		// TlosServer.getTlosParameters().getJobCommand().length != 0) {
		// TlosServer.println(LocaleMessages.getString("TlosServer.71") + " " +
		// TlosServer.getTlosParameters().getJobCommand());
		...
		if (osName.equals(OS_WINDOWS7) || osName.equals(OS_WINDOWS_VISTA) || osName.equals(OS_WINDOWS_2003)) {
			String[] tmpCmd = new String[3];
			tmpCmd[0] = jobCommand;
			tmpCmd[1] = "";
			tmpCmd[2] = "";
			cmd = tmpCmd;
		} else if (osName.equals(OS_WINDOWS_NT) || osName.equals(OS_WINDOWS_XP)) {
			String[] tmpCmd = new String[3];
			tmpCmd[0] = "cmd.exe";
			tmpCmd[1] = "/C";
			tmpCmd[2] = jobCommand;
		...
		if (!osValid) {
			...
			// TlosServer.println(LocaleMessages.getString("LicenseManager.19") + " " +
			// osName);
		}
