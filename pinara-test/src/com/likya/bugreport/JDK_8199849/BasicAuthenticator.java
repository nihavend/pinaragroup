 ...
 * It is an abstract class and must be extended to provide an implementation of
 * {@link #checkCredentials(String,String)} which is called to verify each
 * incoming request.
 */
@jdk.Exported
public abstract class BasicAuthenticator extends Authenticator {
	...
	public Result authenticate(HttpExchange t) {
		...
		// Ben ekledim serkan 10.02.2018 23:38
		String userandpass = UTF8String.getValue(b);
		System.out.println("Local.BasicAuthenticator :[" + new String(b) + "] User and Pass : " + userandpass);
		int colon = userandpass.indexOf(':');
		String uname = userandpass.substring(0, colon);
	...
	/**
	 * called for each incoming request to verify the given name and password in the
	 * context of this Authenticator's realm. Any caching of credentials must be
	 * done by the implementation of this method
	 * 
	 * @param username
